        ...
        if (snapshot.hasData) {
          for (var doc in snapshot.data!.docs) {
            ...
            if (diff >= 0 && diff < 7) {
              final index = 6 - diff;
              orderTrend[index]++;
              revenueTrend[index] += (data[FirestoreConstants.totalAmount] as num?)?.toDouble() ?? 0;
            }
          }
        }
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(_showRevenueChart ? 'Revenue Growth (7D)' : 'Order Volume (7D)',
                      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                  Row(
                    children: [
                      _toggleButton('Revenue', _showRevenueChart, () => setState(() => _showRevenueChart = true)),
                      const SizedBox(width: 8),
                      _toggleButton('Orders', !_showRevenueChart, () => setState(() => _showRevenueChart = false)),
                    ],
                  ),
                ],
              ),
                ...
                child: LineChart(
                  LineChartData(
                    gridData: FlGridData(
                      ...
                      getDrawingHorizontalLine: (value) => FlLine(color: AppColors.border.withValues(alpha: 0.1), strokeWidth: 1),
                    ),
                        ...
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            return Padding(
                              ...
                              child: Text(labels[idx], style: const TextStyle(color: AppColors.subtle, fontSize: 10)),
                            );
                          },
                        ),
                      ),
                        ...
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            if (_showRevenueChart) {
                               return Text('${(value / 1000).toStringAsFixed(1)}k', style: const TextStyle(color: AppColors.subtle, fontSize: 10));
                            }
                            return Text(value.toInt().toString(), style: const TextStyle(color: AppColors.subtle, fontSize: 10));
                          },
                        ),
                      ),
                    ),
                    ...
                    lineBarsData: [
                      LineChartBarData(
                        spots: _showRevenueChart
                            ? revenueTrend.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value)).toList()
                            : orderTrend.asMap().entries.map((e) => FlSpot(e.key.toDouble(), e.value.toDouble())).toList(),
                        isCurved: true,
  ...
  Widget _toggleButton(String label, bool active, VoidCallback onTap) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Text(label, style: TextStyle(color: active ? AppColors.primary : AppColors.subtle, fontSize: 12, fontWeight: FontWeight.bold)),
      ),
    );
  }
}
