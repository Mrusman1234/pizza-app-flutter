...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void initState() {
    super.initState();
    // Initialize dark mode state if needed
  }
  ...
  void _updatePreference(String type, bool value) async {
    ...
    try {
      if (type == 'push') {
        await _firestoreService.updateNotificationPreferences(user.uid, push: value);
      } else if (type == 'sms') {
        await _firestoreService.updateNotificationPreferences(user.uid, sms: value);
      } else if (type == 'email') {
        await _firestoreService.updateNotificationPreferences(user.uid, email: value);
      }
      // Refresh user data to update UI
      await authProvider.fetchUserData(null);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error updating preference: $e')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final smsEnabled = user?.smsEnabled ?? false;
    final emailEnabled = user?.emailEnabled ?? true;
                ...
                SliverPadding(
                  ...
                  sliver: SliverList(
                    delegate: SliverChildListDelegate([
                      ...
                      const SizedBox(height: 8),
                      _buildNotifications(context, push: pushEnabled, sms: smsEnabled, email: emailEnabled),
    ...
    return Container(
      ...
      child: Column(
        children: [
            ...
            children: [
              Container(
                ...
                child: ClipOval(
                  ...
                  : Icon(Icons.person, size: 48, color: isDark ? AppColors.subtle : Colors.grey),
                ),
              ),
              Positioned(
                bottom: 0,
          ...
          Text(
            user?.email ?? 'john.doe@pizzahub.com',
            style: TextStyle(
              fontSize: 14,
  ...
  Widget _buildAccountTile(IconData icon, String label, BuildContext context,
      {bool isLast = false, bool isPrimary = false, bool showArrowForward = false, VoidCallback? onTap}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final primary = AppColors.primary;
  ...
  Widget _buildNotifications(BuildContext context, {required bool push, required bool sms, required bool email}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Container(
      decoration: BoxDecoration(
        color: isDark ? AppColors.card : Colors.white,
      ...
      child: Column(
        children: [
          _buildToggleTile(
            ...
            'Push Notifications',
            push,
            (val) => _updatePreference('push', val),
          ),
          _buildToggleTile(
            Icons.sms,
            'SMS Alerts',
            sms,
            (val) => _updatePreference('sms', val),
          ),
          _buildToggleTile(
            Icons.mail,
            'Email Updates',
            email,
            (val) => _updatePreference('email', val),
            isLast: true,
          ),
        ],
      ),
      ...
      child: Column(
        children: [
          _buildPreferenceTile(
            ...
            showChevron: true,
            onTap: () {
              // Navigation to language selection if screen exists, else placeholder
          ...
          _buildPreferenceTile(
            Icons.delete_sweep,
            'Clear Cache',
            trailing: const Text('Dynamic', style: TextStyle(color: AppColors.muted)),
            isLast: true,
            onTap: () async {
  ...
  void _showDeleteAccountDialog(BuildContext context, AppAuthProvider authProvider) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: AppColors.card,
        ...
        content: const Text("This action is permanent and cannot be undone. All your data will be deleted.", style: TextStyle(color: AppColors.subtle)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text("Cancel", style: TextStyle(color: AppColors.muted)),
